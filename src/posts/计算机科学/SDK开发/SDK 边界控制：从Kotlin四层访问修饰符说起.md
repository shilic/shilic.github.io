...
> [!IMPORTANT]
>
> 随着软件项目的庞大，模块化开发成为必然，高内聚低耦合成为评价一个系统的重要标准。一个模块只做一件事，我们会把一些通用的代码分离出来，到其他项目中进行单独的维护，别人再通过依赖的方式使用。这就是SDK的作用。

一个好的系统是各模块可以随意更换，模块功能不会受到影响；并且允许用户自定义行为，而不破坏内部逻辑。

但是今天，咱们不讲设计模式，先讲最基础的：SDK的访问边界————**什么该暴露，什么该隐藏，什么该允许子类碰，什么该在模块内共享但对外封死。** 这四个问题的答案，决定了你的 SDK 是灵活还是脆弱，是能放手重构还是碰一下都怕。
...
推到极致：SDK 的核心是什么？接口。你暴露给使用者的每一个类、每一个方法，本质上都是在签一份 API 契约——"这个签名我会长期支持"。所以 `public` 不是省事的义项，而是你需要有意识确认的承诺：**我确定这个 API 应该公开，我承担长期兼容的责任。**如果`SDK`的开发者擅自修改了这个`public`方法的签名，那么使用者更新了之后，将会面对一堆的爆红报错。所以说，才称`public`是一份契约，你不能随意更改这个方法的签名，这是承诺。
...
不用凭空编一个例子。直接看 `Kotlin` 标准库——你每天都在用的 `Regex`，背后就是一部 `private` 的教科书：
...
>
> 面试官这样问，你这样糊弄，面试官点点头，反正两边都糊弄，这事就算这样过去了。你不觉得很扯吗？
>
> 完了入职之后，`public`一把梭哈。`protected`的应用场景，估计你想破头也搞不清楚。

`protected` 有一个其他三个修饰符都做不到的精确用途：**模板方法模式**——设计一个算法的骨架，将某些步骤留给子类实现，同时保证这些步骤**只有子类能调用，外部绕不过模板方法直接调**。
