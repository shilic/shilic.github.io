...
> [!NOTE]
>
> **一句话定义**：CI/CD 不是“自动化工具”，而是 SDK 对使用者的**信任担保机制**——每一次发布都经过验证，每一个版本都可追溯，每一次变更都透明。

### 为什么 CI/CD 对 SDK 比对普通项目更重要

普通业务代码的 CI/CD 是为了**效率**——让团队更快地交付功能。SDK 的 CI/CD 是为了**信任**——让使用者敢于升级你的版本。

两者的区别：

| 维度             | 普通业务项目       | SDK 项目                                 |
| ---------------- | ------------------ | ---------------------------------------- |
| **Bug 影响范围** | 一个功能、一个页面 | 所有使用者的所有项目                     |
| **修复成本**     | 改代码、重新部署   | 发新版本 → 使用者升级 → 使用者重新测试   |
| **版本升级意愿** | 无所谓，随时可升   | 使用者**不敢轻易升级**，除非你证明兼容性 |
| **信任建立方式** | 上线验证           | 每一次 CI 通过的绿勾 + 干净的 CHANGELOG  |

**SDK 的本质是“信任商品”**——使用者把代码交给你，相信你不会破坏他们的项目。CI/CD 就是你证明这一点的手段。

------

### CI 做什么：守住三条底线

#### 1. 兼容性底线：自动验证“旧代码在新 SDK 上能否编译运行”

这是 SDK 特有的 CI 任务，普通项目不需要。每次提交，CI 自动拉取上一个版本的调用方示例代码，用当前 SDK 编译运行：
...
jobs:
  compatibility-check:
    ...
    steps:
      ...
      - name: 用上一个版本的调用方代码编译
        run: |
          git checkout v1.0.0 -- tests/compatibility-test
          ./gradlew :tests:compatibility-test:compileKotlin
```

如果这一步红了，说明你做了不兼容的改动——要么修复，要么升主版本。**不兼容的版本绝不能被发布出去。**

#### 2. 质量底线：自动化测试 + 静态扫描

```
每次提交 → 编译 → 单元测试(≥80%) → 集成测试 → 静态代码扫描(依赖冲突/安全漏洞) → 示例代码编译
```

**任何一个环节失败，PR 不能合并。** 这不是“建议”，是硬性门禁。

#### 3. 安全底线：依赖漏洞扫描

SDK 每引入一个依赖，就替使用者承担了一份安全风险。CI 中必须加入 `OWASP Dependency Check` 或 `Trivy` 等工具，扫描所有依赖是否存在已知漏洞。
...
CHANGELOG 也从 commit message 自动生成，**不允许“发布完了再补”**。

#### 2. 发布即验证：确保发布出去的制品是可用的

发布到 Maven Central / npm 之前，CI 自动执行：

- 用发布包创建一个全新的空项目，跑一遍快速开始示例
- 验证包签名（PGP）是否正确
- 验证 pom.xml / package.json 中的依赖声明是否完整

**只有全部通过，才执行 `mvn deploy` 或 `npm publish`。**

------

### 没有 CI/CD 的 SDK 是什么样的？

| 场景             | 后果                                                         |
| ---------------- | ------------------------------------------------------------ |
| 没有兼容性测试   | 使用者升级后发现代码编译不过，回退版本，从此不敢升级         |
| 没有自动版本管理 | 两次发布版本号相同，使用者分不清哪个是新版                   |
| 没有 CHANGELOG   | 使用者不知道升级了什么，只能赌一把                           |
| 手动发布         | 忘了更新版本号、忘了签 PGP、忘了更新 README……                |
| 没有依赖扫描     | 引入了一个有 CVE 漏洞的依赖，所有使用者的项目都被安全部门通报 |

**每一个“忘了”，都是在消费使用者的信任。而信任一旦透支，就再也回不来了。**
...
|                  | 没有 CI/CD                     | 有 CI/CD             |
| ---------------- | ------------------------------ | -------------------- |
| 使用者升级心态   | “又发版了？先看看别人踩坑没……” | “绿勾通过了？升！”   |
| 发现不兼容的时间 | 使用者在生产环境发现           | CI 在合并前发现      |
| 版本号可靠性     | 靠人记，容易错                 | 自动生成，100% 准确  |
| 发布耗时         | 半小时手动操作，可能出错       | 一键触发，全程自动化 |
| 使用者信任度     | 低                             | 高                   |
