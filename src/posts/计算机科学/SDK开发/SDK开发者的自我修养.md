...
> [!NOTE]
>
> **一句话定义**：`CI/CD` 不是”自动化工具”，而是 `SDK` 对使用者的**信任担保机制**——每一次发布都经过验证，每一个版本都可追溯。

### 为什么 `SDK` 的 CI/CD 比普通项目更重要

普通业务代码的 `CI/CD` 为了**效率**。`SDK` 的 `CI/CD` 为了**信任**——你的一个 `bug` 影响所有使用者的所有项目，使用者不敢轻易升级，除非你证明兼容性。

### CI：三条硬性门禁

每次提交必须通过，任何一步失败 `PR` 不能合并：

| 门禁 | 做什么 |
|---|---|
| **兼容性** | 用上一个版本的调用方代码编译当前 SDK |
| **质量** | 单元测试 ≥ 80% + 集成测试 + 静态扫描 + 示例代码编译 |
| **安全** | 依赖漏洞扫描,`./gradlew dependencyCheckAnalyze`，高危漏洞直接阻断 |
...
| 场景 | 后果 |
|---|---|
| 没有兼容性测试 | 使用者升级后编译不过，回退，从此不敢升级 |
| 没有自动版本管理 | 同一个版本号发了两次，分不清新旧 |
| 没有 `CHANGELOG` | 不知道升级了什么，赌一把 |
| 手动发布 | 忘改版本号、忘签 `GPG`、忘更 `README` |
| 没有依赖扫描 | `SDK` 引入 `CVE` 漏洞，所有使用者被安全部门通报 |

> 💡 **每一个”忘了”，都是在消费使用者的信任。信任一旦透支，就再也回不来了。**
