...
我没有在屎山上继续堆砌，而是花了一周把整个项目的逻辑和功能摸透，补上注释，然后推翻重写。

我问同事：那些被注释的代码是什么意思？是准备之后恢复，还是可以直接删除？

同事说：太久了，我也不记得了。但不删吧——万一有用呢。

这就是问题。

被注释掉的代码，本身没有恶意。写它的那个程序员可能只是暂时跳过了某个功能，可能只是调试时随手关掉了一段逻辑。但项目一交接、时间一拉长，它就变成了无人能解的谜：**能动吗？删了会出事吗？恢复后还能跑吗？**

注释在一开始是注释，在后来者眼里就是地雷。

但本文不展开讲命名、讲架构、讲那些更大的话题。本文只聚焦一个点：**当你需要废弃一个函数、或者标记一段未完成的代码时，正确的做法是什么。**

## 二、只有 `JetBrains` 在乎

在回答"怎么做"之前，先看看各家 IDE 对这件事的态度。
...
我用过的众多 IDE 里，**只有 `IntelliJ IDEA`** 会对你被注释的代码发出黄标警告：

![IDEA提示被注释的代码](./assets/image-20260701145924631.png)

刚开始我觉得这群 `JetBrains` 工程师真是多管闲事——我就注释几行代码，至于吗？

后来项目经得多了，我才明白他们在做什么。**被注释掉的代码**，刚写下的时候没什么大不了；但项目变大、人换了又换，后来者看到这段代码，它就成了一道迷题：**它到底是暂时停用，还是永久废弃，还是干脆是一个忘了删除的旧版本？** 没人知道。**被注释的代码不会说话，但它留下的困惑，足以让维护者抓狂。**

是的，他们用心良苦，为了防止你们写出不规范的代码，他们甚至想要手把手教你。
...
| JavaScript / TypeScript | `/** @deprecated */` | ✅ 编译通过，不写 JSDoc 也没人管 |
...
绝大多数语言，`TODO` 只是一行对人类无效的注释——同事不会天天翻你的注释看。`Rust `的 `todo!()`宏思路接近，但 `Kotlin `把它做成标准库函数 + `Nothing `返回，调用侧更轻，且 `IDE `对 `TODO()` 有额外工具链支持（`TODO `面板、跳转）。`C#` 的 `Visual Studio` 会在任务列表中收集 `// TODO`，但离开了 `VS `就没人看得见。
...
崩溃不是 bug——崩溃是**你故意埋的保险丝**。它强迫你在上线前面对那段没写完的代码，而不是假装它已经完成了。你在开发环境忘了，`TODO()` 会在你第一次点到这里时用 `NotImplementedError` 把你炸回去。
...
这就是工程师的精神：**严谨，有一种完美主义精神，用强制性来规范使用者做出正确的事。**
